<script>
  ...
  function findRetreat(retreat_id) {
    const retreat = data.retreats.find((retreat) => retreat_id === retreat.id);
    return retreat;
  }
</script>
        ...
        <tbody>
          {#each users as user, index (user.id)}
            <tr>
              ...
              <td>
                {dayjs(user.check_in_date).format("DD MMM HH:mm")}
              </td>
              <td>
                {#if user.check_out_date != ""}
                  {dayjs(user.check_out_date).format("DD MMM HH:mm")}
                {:else}
                  <button
                    type="button"
              ...
              <td>
                <div class="input-group">
                  <input
                    ...
                    onchange={(event) => {
                      user.leave_date = event.target.value;
